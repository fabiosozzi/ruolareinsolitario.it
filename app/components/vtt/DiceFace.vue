...
<template>
  <!-- d4: triangolo |
     -->
  <svg v-if="type === 'd4'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
    ...
    <text x="16" y="22" text-anchor="middle" fill="white" font-size="11" font-weight="bold" font-family="inherit">{{ value }}</text>
  </svg>

  <!-- d6: quadrato 2D -->
  <svg v-else-if="type === 'd6'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
    ...
    <text x="16" y="20" text-anchor="middle" fill="white" font-size="11" font-weight="bold" font-family="inherit">{{ value }}</text>
  </svg>

  <!-- d8: rombo -->
  <svg v-else-if="type === 'd8'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
  ...
  <!-- d10: pentagono -->
  <svg v-else-if="type === 'd10'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
  ...
  <!-- d12: esagono -->
  <svg v-else-if="type === 'd12'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
  ...
  <svg v-else-if="type === 'd20'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
    <polygon points="28,16 22,26 10,26 4,16 10,6 22,6" :fill="c.bg" :stroke="c.border" stroke-width="1.5" stroke-linejoin="round" />
    ...
    <circle cx="16" cy="16" r="2" :fill="c.light" />
    <text x="16" y="20" text-anchor="middle" fill="white" font-size="11" font-weight="bold" font-family="inherit">{{ value }}</text>
  </svg>

  <!-- d100: cerchio -->
  <svg v-else-if="type === 'd100'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
    ...
    <text x="16" y="20" text-anchor="middle" fill="white" font-size="10" font-weight="bold" font-family="inherit">{{ value }}</text>
  </svg>

  <!-- fal-cd: Challenge Die stile Pip-Boy -->
  <svg v-else-if="type === 'fal-cd'" viewBox="0 0 32 32" class="w-8 h-8 shrink-0">
  ...
  <!-- fallback -->
  <span v-else class="inline-flex items-center justify-center w-8 h-8 rounded text-xs font-bold shrink-0" :style="{ backgroundColor: c.bg, color: 'white' }">
    {{ value }}
  </span>
</template>
