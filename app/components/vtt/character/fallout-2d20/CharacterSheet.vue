...
<template>
  <div class="bg-black/70 backdrop-blur-sm border border-green-700/50 rounded-lg p-2.5 text-white h-full flex flex-col text-sm gap-2">
    <div class="flex items-center gap-1 border-b border-green-700/30 pb-1.5">
      <h3 class="text-sm font-heading font-semibold text-green-400 flex-1 truncate">
        {{ t('vtt.character.title') }}
      </h3>
      <button class="px-2 py-0.5 text-xs font-heading bg-green-700/60 hover:bg-green-600/70 border border-green-700/50 rounded text-green-100 cursor-pointer transition-all focus-visible:ring-2 focus-visible:ring-green-400/60 active:scale-[0.98]" @click="addNew">+</button>
    </div>
    ...
    <div class="flex flex-col gap-1 flex-1 overflow-y-auto">
      ...
      >
        <span class="flex-1 truncate text-xs font-heading">{{ c.name || '—' }}</span>
        <button class="text-green-400/50 hover:text-green-300 text-xs cursor-pointer" :aria-label="t('vtt.character.viewDetail')" @click.stop="open(c.id)">◎</button>
      </div>

      <div v-if="!characters.length" class="flex-1 flex items-center justify-center text-xs text-green-400/40 italic">
